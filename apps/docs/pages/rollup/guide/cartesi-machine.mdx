...
1. **Node.js for riscv64** — Node.js does not publish official riscv64 builds. Cartesi maintains its own (Ubuntu noble based), published on Docker Hub as [`cartesi/node`](https://hub.docker.com/r/cartesi/node) — e.g. `cartesi/node:24-noble-slim`.
...
```dockerfile
FROM --platform=linux/riscv64 cartesi/node:24-noble-slim
...
Each input file uses the same `EvmAdvance` encoding as the [host mock](/rollup/guide/testing#generating-an-advance-input); outputs and reports are stored to files as the machine emits them.

For a complete, working example — building the rootfs, feeding inputs, verifying outputs byte-for-byte — see the package's own end-to-end test harness: [`test/machine/`](https://github.com/tuler/deroll/tree/main/packages/bindings/rollup/test/machine).

## Limits to keep in mind

- **One `Rollup` instance at a time.** The device is exclusive; a second `new Rollup()` throws `-EBUSY` until the first is [closed](/rollup/reference/close).
