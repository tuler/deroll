...
# 3. pack @deroll/rollup and @deroll/codec and stage the dapp directory
# (the prebuild rides along in the rollup tarball; --ignore-scripts skips
# node-gyp-build's host-side install)
if [ -z "${SKIP_ROOTFS:-}" ]; then
    echo "==> staging dapp with packed tarballs"
    CONTEXT="$WORK/context"
    rm -rf "$CONTEXT"
    mkdir -p "$CONTEXT/dapp"
    TARBALL=$(npm pack --pack-destination "$WORK" | tail -1)
    CODEC_TARBALL=$(npm pack ../../app/codec --pack-destination "$WORK" | tail -1)
    npm install --prefix "$CONTEXT/dapp" --ignore-scripts --omit=dev \
        "$WORK/$TARBALL" "$WORK/$CODEC_TARBALL"
