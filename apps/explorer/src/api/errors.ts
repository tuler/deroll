...
export function isRpcResponseError(error: unknown): boolean {
  return error instanceof BaseError && error.walk((e) => e instanceof RpcRequestError) !== null
}
...
export function apiErrorMessage(error: unknown, server?: string): string {
  if (error instanceof BaseError) {
    if (error.walk((e) => e instanceof HttpRequestError) && !isRpcResponseError(error)) {
      return (
