...
test("advance request, outputs and reports", async () => {
    ...
    assert.ok(
        voucher.includes(addressWord(destination)),
        "voucher contains destination",
    );
    ...
    // reports are raw
    assert.deepEqual(
        fs.readFileSync(path.join(dir, "advance.report-0.bin")),
        reportPayload,
    );
...
test("delegate call voucher", async () => {
    ...
    assert.equal(
        output.subarray(0, 4).toString("hex"),
...
test("merkle save, reset and load", async () => {
    ...
    assert.throws(
        ...
        (error) =>
            error instanceof RollupError &&
            error.syscall === "cmt_rollup_load_merkle",
    );
    another.close();
});
...
test("input validation", async () => {
    ...
    assert.throws(
        () => rollup.emitVoucher({ destination: "0x1234" }),
        /destination must be 20 bytes/,
    );
    assert.throws(
        () => rollup.emitVoucher({ destination: "not-hex" }),
        TypeError,
    );
    assert.throws(
        () =>
