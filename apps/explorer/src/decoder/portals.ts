...
const uint256 = (payload: Hex, start: number) =>
    hexToBigInt(slice(payload, start, start + 32));
...
const orUndefined = (data: Hex): Hex | undefined =>
    data === "0x" ? undefined : data;
...
export function decodePortalDeposit(
    ...
    switch (portal) {
        ...
        case "ERC721Portal": {
            ...
            try {
                const [base, exec] = decodeAbiParameters(
                    BYTES_PAIR,
                    tail(payload, 72) ?? "0x",
                );
                return {
        ...
        case "ERC1155BatchPortal": {
            ...
            try {
                const [ids, values, base, exec] = decodeAbiParameters(
                    BATCH_DATA,
                    tail(payload, 40) ?? "0x",
                );
                return {
