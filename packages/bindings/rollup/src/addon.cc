...
Rollup::Rollup(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Rollup>(info) {
    ...
    int rc = cmt_rollup_init(&rollup_, nullptr);
    if (rc < 0) {
...
Napi::Value Rollup::WaitForInput(const Napi::CallbackInfo &info) {
    ...
    cmt_buf_t out{};
    int rc = cmt_rollup_wait_for_input(&rollup_, accept, &out);
    if (rc < 0) {
        errno_error(env, "cmt_rollup_wait_for_input", rc).ThrowAsJavaScriptException();
        return env.Undefined();
    }
    const char *type = nullptr;
    if (rc == HTIF_YIELD_REASON_ADVANCE) {
        type = "advance";
    } else if (rc == HTIF_YIELD_REASON_INSPECT) {
        type = "inspect";
    } else {
...
Napi::Value Rollup::EmitOutput(const Napi::CallbackInfo &info) {
    ...
    int rc = cmt_rollup_emit_output(&rollup_, cmt_buf_make(length, data));
    if (rc < 0) {
...
Napi::Value Rollup::EmitReport(const Napi::CallbackInfo &info) {
    ...
    int rc = cmt_rollup_emit_report(&rollup_, cmt_buf_make(length, data));
    if (rc < 0) {
...
Napi::Value Rollup::EmitException(const Napi::CallbackInfo &info) {
    ...
    int rc = cmt_rollup_emit_exception(&rollup_, cmt_buf_make(length, data));
    if (rc < 0) {
