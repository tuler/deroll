...
1. The application calls [`waitForInput`](/cmio/reference/wait-for-input), telling the machine whether it **accepts or rejects** the previous request.
...
Advance handlers receive the request with its **raw, undecoded payload**. Parse it with [`decodeAdvance`](/cmio/reference/decode-advance) to get the input metadata and inner payload:
...
`run` is a thin convenience. If you prefer explicit control — custom dispatch, graceful shutdown, request batching — call [`waitForInput`](/cmio/reference/wait-for-input) directly:
