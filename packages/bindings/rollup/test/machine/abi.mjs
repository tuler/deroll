...
// application-defined bytes32 output tag; must match the constant in app.mjs
// (which runs inside the machine and cannot import this file).
export const APP_CONTEXT = `0x${'00'.repeat(31)}42`;
...
export function encodeNotice({ appContext, payload }) {
    return Buffer.concat([
        Buffer.from(SELECTOR.notice, 'hex'),
        Buffer.from(appContext.slice(2), 'hex'),
        word(2 * 32), // offset of the payload `bytes` field
        word(payload.length),
        pad32(payload),
    ]);
}

export function encodeCallVoucher({ destination, appContext, value, payload }) {
    return Buffer.concat([
        Buffer.from(SELECTOR.callVoucher, 'hex'),
        Buffer.from(appContext.slice(2), 'hex'),
        addressWord(destination),
        word(value),
        word(4 * 32), // offset of the payload `bytes` field
        word(payload.length),
        pad32(payload),
    ]);
}
