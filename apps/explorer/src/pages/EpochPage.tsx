...
function HashList({ hashes }: { hashes: string[] | null }) {
    ...
    return (
        <ol className="space-y-1 font-mono text-xs">
            {hashes.map((h, i) => (
                // biome-ignore lint/suspicious/noArrayIndexKey: merkle proof entries are positional
                <li key={i} className="flex gap-2">
                    <span className="w-6 text-right text-slate-400 dark:text-slate-500">
                        {i}
                    </span>
                    <Hex value={h} full />
                </li>
...
export function EpochPage() {
    ...
    const epoch = useEpoch({
        ...
        epochIndex: parseUintParam(epochIndex),
    });

    if (epoch.isPending) return <Spinner />;
    if (epoch.isError) return <ErrorBox error={epoch.error} />;
    const e = epoch.data;
        ...
        <div className="space-y-4">
            ...
            >
                <KV
                    rows={[
                        ...
                        [
                            "Outputs merkle root",
                            <Hex value={e.outputsMerkleRoot} full />,
                        ],
                        ["Commitment", <Hex value={e.commitment} full />],
                        [
                            "Claim transaction",
                            <TxHash value={e.claimTransactionHash} full />,
                        ],
                        e.stagedAtBlock
                            ? ["Staged at block", formatUint(e.stagedAtBlock)]
                            : null,
                        e.tournamentAddress
                            ? [
                                  "Tournament",
            ...
            <Section title="Explore this epoch">
                <div className="flex flex-wrap gap-2">
                    <QuickLink
                        to={`${base}/inputs?epoch=${epochIndex}`}
                        label="Inputs"
                    />
                    <QuickLink
                        to={`${base}/outputs?epoch=${epochIndex}`}
                        label="Outputs"
                    />
                    <QuickLink
                        to={`${base}/reports?epoch=${epochIndex}`}
                        label="Reports"
                    />
                    <QuickLink
                        to={`${base}/tournaments?epoch=${epochIndex}`}
                        label="Tournaments"
                    />
                </div>
            </Section>
            ...
            <div className="space-y-2">
                <Collapsible
                    label={`Commitment proof (${e.commitmentProof?.length ?? 0} hashes)`}
                >
                    <HashList hashes={e.commitmentProof} />
                </Collapsible>
                <Collapsible
                    label={`Outputs merkle proof (${e.outputsMerkleProof?.length ?? 0} hashes)`}
                >
