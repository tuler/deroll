...
async function importDecoder(url: string): Promise<DecoderModule> {
    ...
    try {
        mod = (await import(/* @vite-ignore */ importUrl)) as Record<
            string,
            unknown
        >;
    } catch (err) {
    ...
    return validateDecoderModule(
        typeof mod.version === "number" ? mod : mod.default,
    );
}
...
export function validateDecoderModule(mod: unknown): DecoderModule {
    ...
    if (candidate?.version !== 1) {
        throw new Error(
            `Unsupported decoder version ${String(candidate?.version)} (expected 1).`,
        );
    }
    ...
    if (!DECODE_METHODS.some((m) => typeof candidate[m] === "function")) {
        throw new Error(
            `The module exports none of the decode methods (${DECODE_METHODS.join(", ")}).`,
        );
    }
