...
import type { AdvanceRequestHandler } from "@deroll/core";
...
`Rollup.run` takes a single advance handler. To run several, compose them with [chain](/app/chain), which presents the input to each in order until one accepts it:
...
Use [broadcast](/app/broadcast) instead when every handler should see the input even after one has accepted it.
