...
import { parseUintParam, formatDate, formatUint, isZeroHex, shortHex, uintToDecimal } from '../lib/format'
...
export function MatchPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <KV
          rows={[
            ...
            [
              'Tournament',
              <Hex value={m.tournamentAddress} full to={`${base}/tournaments/${m.tournamentAddress}`} />,
            ],
            [
              'Epoch',
              <Link
            ...
            ['Commitment one', <Hex value={m.commitmentOne} full />],
            ['Commitment two', <Hex value={m.commitmentTwo} full />],
            ...
            ['Resolution', <StatusBadge status={m.deletionReason} />],
            deleted && m.deletionBlockNumber !== null && m.deletionBlockNumber !== 0n
              ? ['Resolved at block', formatUint(m.deletionBlockNumber)]
              : null,
            deleted && !isZeroHex(m.deletionTxHash)
              ? ['Resolution tx', <TxHash value={m.deletionTxHash} full />]
              : null,
      ...
      <Section title={`Match advances (${advances.data?.pagination.totalCount ?? '…'})`}>
        <DataTable
          columns={[
            { header: 'Other parent', cell: (a) => <Hex value={a.otherParent} /> },
            { header: 'Left node', cell: (a) => <Hex value={a.leftNode} /> },
            { header: 'Block', align: 'right', cell: (a) => formatUint(a.blockNumber) },
            { header: 'Tx', cell: (a) => <TxHash value={a.txHash} /> },
            { header: 'Observed', cell: (a) => formatDate(a.createdAt) },
          ]}
          rows={advances.data?.data}
          rowKey={(a) => `${a.otherParent}-${a.leftNode}-${a.blockNumber}`}
