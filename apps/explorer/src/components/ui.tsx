...
export function Hex({
    ...
    return (
        <span
            className="inline-flex items-center gap-1 font-mono text-[13px] break-all"
            title={value}
        >
            {inner}
            <button
...
export function Pill({
    ...
    const colorClass =
        color && Object.hasOwn(PILL_COLORS, color)
            ? PILL_COLORS[color as TagColor]
            : GRAY;
    return (
        <span
            title={title}
            ...
            {rows
                ...
                .map(([key, value], i) => (
                    ...
                    <div key={i} className="contents">
                        ...
                        <dd className="text-sm text-slate-900 min-w-0 dark:text-slate-100">
                            {value}
                        </dd>
                    </div>
                ))}
