...
1. an [anvil](https://book.getfoundry.sh/anvil/) instance running at http://localhost:6751/anvil which contains all Cartesi infrastructure [smart contracts](https://docs.cartesi.io/cartesi-rollups/1.5/rollups-apis/json-rpc/overview/) deployed to it, and also a few test contracts. You can see the addresses of these contracts by executing the `cartesi address-book` command.

2. a [JSON-RPC]() server running at http://localhost:6751/rpc which you can use to query outputs produced by your application.

3. an inspect server running at http://localhost:6751/inspect/ which you can use to simulate an input, or query read-only computations from your application.
...
:::info
The address of the InputBox can also be obtained by executing the command `cartesi address-book --json | jq -r '.InputBox'` and the address of the application with the command `cartesi address-book --json | jq -r '.Application'`.
:::
...
:::code-group
```bash [command]
$ curl -s http://localhost:6751/inspect/tuler
