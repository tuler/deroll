...
`addAdvanceHandler` and `addInspectHandler` are gone: `Rollup.run` takes one handler of each kind. To run several advance handlers, compose them with [chain](/app/chain):
...
The `broadcastAdvanceRequests` option is now [broadcast](/app/broadcast), chosen per group of handlers rather than for the whole application.
