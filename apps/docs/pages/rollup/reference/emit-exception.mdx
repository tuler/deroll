...
Prefer the gentler paths first: return `false` from a [`run`](/rollup/reference/run) handler to reject an input, or throw (which `run` converts to a report plus rejection). `emitException` is the last resort for "this application cannot meaningfully continue with this request".
