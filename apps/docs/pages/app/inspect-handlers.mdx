...
import type { InspectRequestHandler } from "@deroll/core";
...
report. To answer queries from several handlers, call them from one — there is
no verdict to short-circuit on, so unlike [chain](/app/chain) there is nothing
to compose.
