...
import {
    useDecodedPayload,
    type DecodeProps,
} from "../decoder/useDecodedPayload";
...
function decodedToText(result: DecodeResult, indent?: number): string {
    ...
    if (result.data !== undefined)
        return JSON.stringify(result.data, bigintSafe, indent);
...
function TagPills({ tags }: { tags: Tag[] }) {
    ...
    return (
        <span className="inline-flex flex-wrap items-center gap-1">
            {tags.map((t, i) => (
                // biome-ignore lint/suspicious/noArrayIndexKey: tags carry no stable identity; their order is the identity
    ...
    return (
        <div className="space-y-1.5">
            ...
            {mode === "decoded" &&
                result &&
                (tags.length > 0 ||
                    (result.summary && result.data !== undefined)) && (
