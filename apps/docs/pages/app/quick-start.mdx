...
If you select the default options (no wallet library, more on that later), you will get the following `src/index.ts` file.
...
```ts [src/index.ts]
        // log incoming advance request
        advance: (request) => {
            const { type, payload, ...metadata } = request; // [!code focus]
            console.log(metadata); // [!code focus]
            console.log(payload.toString()); // [!code focus]
            return true;
        },
```
:::
...
```ts [src/index.ts]
        // log incoming inspect request
        inspect: (request) => {
            console.log(toHex(request.payload)); // [!code focus]
        },
```
:::

If you check in the `cartesi logs` log you will see the output above, which is the result of the `console.log` in the inspect handler.
